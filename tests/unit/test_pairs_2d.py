...
def small_cat_2d():
    ...
    part_ids = rng.integers(0, 3, n).astype(np.int32)
    rp_bins = np.array([1.0, 5.0, 15.0])
    pi_bins = np.array([0.0, 5.0, 15.0, 30.0])
    return coords, part_ids, box, rp_bins, pi_bins
...
def uniform_cat_2d():
    ...
    return coords, part_ids, box, rp_bins, pi_bins, k
...
class TestCountPairs2D:
    def test_matches_brute_force_auto(self, small_cat_2d):
        ...
        auto, _ = count_pairs_2d(coords, part_ids, rp_bins, pi_bins, box)
        bf_auto, _ = _bf_2d(coords, part_ids, rp_bins, pi_bins, box)
        assert np.allclose(auto, bf_auto)

    def test_matches_brute_force_cross(self, small_cat_2d):
        coords, part_ids, box, rp_bins, pi_bins = small_cat_2d
        _, cross = count_pairs_2d(coords, part_ids, rp_bins, pi_bins, box)
        _, bf_cross = _bf_2d(coords, part_ids, rp_bins, pi_bins, box)
    ...
    def test_all_same_partition_zero_cross(self, small_cat_2d):
        coords, _, box, rp_bins, pi_bins = small_cat_2d
        part_ids = np.zeros(len(coords), dtype=np.int32)
        auto, cross = count_pairs_2d(coords, part_ids, rp_bins, pi_bins, box)
        assert np.all(cross == 0.0)

    def test_all_distinct_partitions_zero_auto(self, small_cat_2d):
        coords, _, box, rp_bins, pi_bins = small_cat_2d
...
class TestCompute2PCF:
    def test_output_keys(self, uniform_cat_2d):
        ...
        result = compute_2pcf(coords, part_ids, rp_bins, pi_bins, box, k, k)
    ...
    def test_invalid_m_exceeds_k(self, uniform_cat_2d):
        ...
        with pytest.raises(ValueError):
            compute_2pcf(coords, part_ids, rp_bins, pi_bins, box, k, k + 1)

    def test_invalid_m_is_zero(self, uniform_cat_2d):
        coords, part_ids, box, rp_bins, pi_bins, k = uniform_cat_2d
        with pytest.raises(ValueError):
            compute_2pcf(coords, part_ids, rp_bins, pi_bins, box, k, 0)
